...
- Layer flow: `components` → `hooks` → `lib` — transitive: a layer may import **any** layer after it, unless the target narrows its importers.
- **Before adding, moving, or renaming any file** — placement, module boundaries, unit shapes, ownership, naming: read [docs/architecture-handbook.md](docs/architecture-handbook.md) (generated from the same blueprint — always current).
