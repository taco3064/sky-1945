...
When the contract and the code disagreed, the code moved.
...
`emitLint(blueprint)` spread inside it is untouched: every structural rule still
comes from `blueprint.config.mjs` at runtime. Worth stating plainly, because on a
project whose claim is that the contract is machine-enforced, *which* machine
enforces it is part of the finding.
