...
> **This is the layer-first snapshot** — tag `layer-first`, kept beside the
> module-first `main` for comparison. It is `main` after blueprint's module-first →
> layer-first transformation, and the tree it produced is identical, file for file, to
> the 4.0 layer-first commit made before the move (`6a13166`). The findings below were
> written while `main` was module-first.
...
The source is layer-first. Layers run one way — `pages → containers → components →
hooks → contexts → engine`. The budgets on lines, function length, parameters and
complexity come from the preset, and the handbook lists them all.
...
- **The boundaries are machine-checked, not remembered.** `matter-js` and
  `requestAnimationFrame` are reachable only from `engine`; no `components` file
  imports the engine. Both fail lint.
