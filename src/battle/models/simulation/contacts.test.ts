...
import { type Enemy, createEnemy, roundSchedule } from '../enemies';
...
function bullet(world: World, side: 'player' | 'enemy', damage = 11.25): Bullet {
  const created = createBullet(nextId(world), {
    side,
    x: 100,
    y: 100,
    heading: side === 'player' ? -90 : 90,
    speed: 300,
    damage,
  });
...
describe('player bullet + enemy aircraft', () => {
  it('removes the bullet and damages the enemy, in either order', () => {
    ...
    resolveContacts(world, [
      [as.bullet(shot), as.enemy(target)],
      [as.enemy(target), as.bullet(other)],
    ]);
  ...
  it('lets one bullet damage two enemies it starts touching in the same pass', () => {
    ...
    resolveContacts(world, [
      [as.bullet(shot), as.enemy(first)],
      [as.bullet(shot), as.enemy(second)],
    ]);
  ...
  it('bursts a destroyed enemy only once', () => {
    ...
    resolveContacts(world, [
      [as.bullet(first), as.enemy(target)],
      [as.bullet(second), as.enemy(target)],
    ]);
...
describe('player bullet + boss', () => {
  ...
  it('removes the boss and its beam together, with a large burst at its centre', () => {
    ...
    resolveContacts(world, [
      [as.bullet(first), as.boss(boss)],
      [as.bullet(second), as.boss(boss)],
    ]);

    expect(world.boss).toBeNull();
    expect(boss.beam).toBeNull();

    expect([world.collisions.bodies.has(boss.id), world.collisions.bodies.size])
      .toEqual([false, 1]);
...
describe('player contacts', () => {
  it('costs a life on an enemy bullet with an ally burst; the bullet flies on', () => {
    const world = setup();
    ...
    expect(world.player)
      .toMatchObject({ x: 270, y: 1020, flyingIn: true, protectedUntil: 13 });
  ...
  it('ignores enemies and boss beams destroyed earlier in the same pass', () => {
    const world = setup();
    const target = enemy(world);
    const boss = arrivedBoss(world);
...
describe('everything else', () => {
  it('does nothing', () => {
    ...
    expect([world.lives, world.bullets.length, world.enemies.length, first.hp, boss.hp])
      .toEqual([3, 2, 2, 20, 900]);
  });
});
