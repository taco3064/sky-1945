...
it('fires ten radial bullets whatever the heading', () => {
  expect(patternHeadings('radial', 90))
...
it('creates one enemy bullet per heading from the origin with fresh ids', () => {
  ...
  const bullets = fireVolley(
    { pattern: 'spread', x: 100, y: 226, heading: 90, speed: 260, damage: 10 },
    () => ++id,
  );
  ...
  bullets.forEach((bullet) => {
    expect(bullet).toMatchObject({ side: 'enemy', x: 100, y: 226, damage: 10 });
  });
