...
describe('moveSteering', () => {
  it('steers nowhere inside the 5 px dead zone while the knob still follows', () => {
    const steering = startSteering(1, { x: 100, y: 200 }, 0);

    expect(moveSteering(steering, 103, 203.9))
      .toEqual({ direction: { x: 0, y: 0 }, knob: { x: 3, y: expect.closeTo(3.9, 9) } });
  ...
  it('steers along the offset from 5 px on and remembers the move', () => {
    ...
    expect(moveSteering(steering, 103, 204))
      .toEqual({ direction: { x: 3, y: 4 }, knob: { x: 3, y: 4 } });
  ...
  it('caps the knob offset at 26 px', () => {
    const steering = startSteering(1, { x: 0, y: 0 }, 0);
...
describe('isTap', () => {
  it('is a tap when released within 200 ms without moving 5 px', () => {
    const steering = startSteering(1, { x: 0, y: 0 }, 1000);
