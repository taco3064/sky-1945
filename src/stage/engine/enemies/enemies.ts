...
import type { EnemyBoosts, EnemyField, Flight, Step, Tick } from './types';
...
/** A volley, always at least `ENEMY_BULLET_LEAD` faster than the craft that fired it. */
function volleyFrom(flight: Flight, body: Body, boosts: EnemyBoosts): BulletSpawn[] {
  ...
  // A radial burst leaves the centre; an aimed shape leaves the nose.
...
export function createEnemyField(engine: Engine): EnemyField {
  ...
  const advanceOne = (body: Body, tick: Tick): Step => {
    ...
    // The cadence starts when it reaches the field, not when it spawns outside it.
    if (next.y <= 0) {
      return IDLE;
    }
    ...
    // Scaled by the round's speed, so a faster craft fires proportionally more often.
