...
it('arrives full, hatched and dashed with ARRIVING while shielded', () => {
  const { container } = render(
    <BossHealthBar hp={1350} maxHp={1350} pose="entering" move={null} />,
  );

  const bar = container.firstElementChild as HTMLElement;
  const track = screen.getByRole('progressbar', { name: 'Boss health, shielded' });

  expect(bar.className).toBe('boss-bar boss-bar--shielded');
  expect(track.getAttribute('aria-valuemin')).toBe('0');
  expect(track.getAttribute('aria-valuemax')).toBe('1350');
  expect(track.getAttribute('aria-valuenow')).toBe('1350');
  expect((track.firstElementChild as HTMLElement).style.width).toBe('100%');
  expect([...bar.children].map((child) => child.textContent)).toEqual(['', 'ARRIVING']);
  expect(screen.getByText('ARRIVING').tagName).toBe('P');
});

it('shows the remaining fraction while fighting', () => {
  const { container } = render(
    <BossHealthBar hp={675} maxHp={900} pose="firing" move="spread" />,
  );
...
it('turns low at a quarter or less and clamps below zero', () => {
  const { container, rerender } = render(
    <BossHealthBar hp={225} maxHp={900} pose="recovering" move="ram" />,
  );
...
it('warns ROLL while the boss winds up a beam', () => {
  const { rerender } = render(
    <BossHealthBar hp={900} maxHp={900} pose="winding" move="beam" />,
  );
