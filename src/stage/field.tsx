import { useEffect } from 'react';

import Beam from '~app/stage/components/Beam';
import Boss from '~app/stage/components/Boss';
import type { BossMove, BossPose } from '~app/stage/components/Boss';
import Bullet from '~app/stage/components/Bullet';
import Burst from '~app/stage/components/Burst';
import Enemy from '~app/stage/components/Enemy';
import type { EnemyVariant } from '~app/stage/components/Enemy';
import Fighter from '~app/stage/components/Fighter';
import FrameMeter from '~app/stage/components/FrameMeter';
import HealthBar from '~app/stage/components/HealthBar';
import LifeIcon from '~app/stage/components/LifeIcon';
import SpeedLines from '~app/stage/components/SpeedLines';
import TouchStick from '~app/stage/components/TouchStick';
import { GameProvider } from '~app/stage/contexts/GameContext';
import type { BossSnapshot } from '~app/stage/engine/boss';
import type { CombatSnapshot } from '~app/stage/engine/combat';
import type { EntityKind, EntityRecord } from '~app/stage/engine/world';
import { useEntityRoster } from '~app/stage/hooks/useEntityRoster';
import { useFrameRate } from '~app/stage/hooks/useFrameRate';
import { useGameRound } from '~app/stage/hooks/useGameRound';
import { useHitPoints } from '~app/stage/hooks/useHitPoints';
import { useLives } from '~app/stage/hooks/useLives';
import { usePlayerCombat } from '~app/stage/hooks/usePlayerCombat';
import { usePlayerInput } from '~app/stage/hooks/usePlayerInput';
import { useStageScale } from '~app/stage/hooks/useStageScale';

import styles from './styles.module.css';
import type { FieldProps } from './types';

/** Which silhouette an enemy kind draws with. */
const VARIANTS: Record<string, EnemyVariant> = {
  'enemy-small': 'small',
  'enemy-medium': 'medium',
  'enemy-large': 'large',
};

function variantFor(kind: EntityKind): EnemyVariant {
  return VARIANTS[kind];
}

/** Which component draws a roster entry. Only the boss draws its state as well. */
function draw(entity: EntityRecord, combat: CombatSnapshot, boss: BossSnapshot | null) {
  if (entity.kind === 'player') {
    return (
      <Fighter
        key={entity.id}
        id={entity.id}
        rolling={combat.rolling}
        invulnerable={combat.invulnerable}
        ready={combat.ready}
      />
    );
  }

  if (entity.kind === 'player-bullet' || entity.kind === 'enemy-bullet') {
    return (
      <Bullet key={entity.id} id={entity.id} hostile={entity.kind === 'enemy-bullet'} />
    );
  }

  if (entity.kind === 'enemy-beam') {
    return <Beam key={entity.id} id={entity.id} />;
  }

  if (entity.kind === 'enemy-boss') {
    return (
      <Boss
        key={entity.id}
        id={entity.id}
        pose={(boss?.stance ?? 'entering') as BossPose}
        move={boss?.attack as BossMove | undefined}
        size={boss?.scale}
      />
    );
  }

  if (entity.kind === 'burst' && entity.burst) {
    return (
      <Burst
        key={entity.id}
        id={entity.id}
        size={entity.burst.size}
        tone={entity.burst.tone}
      />
    );
  }

  return <Enemy key={entity.id} id={entity.id} variant={variantFor(entity.kind)} />;
}

/** Everything that watches a running world, so every hook here takes a `World`. */
export default function Field({
  world,
  pace,
  phase,
  onPause,
  onQuit,
  onGameOver,
}: FieldProps) {
  const viewport = useStageScale();
  const { surface, stick } = usePlayerInput(world, onPause);
  const entities = useEntityRoster(world);
  const combat = usePlayerCombat(world);
  const round = useGameRound(world);
  const boss = useHitPoints(world);
  const rate = useFrameRate(world);
  const lives = useLives(world, onGameOver);

  useEffect(() => {
    if (phase === 'playing') {
      world.start();
    } else {
      world.pause();
    }
  }, [world, phase]);

  return (
    <GameProvider world={world}>
      <div ref={viewport} className={styles.viewport}>
        <div className={styles.field}>
          {/* The only thing on the field that is not an entity. */}
          <SpeedLines pace={pace} />

          {entities.map((entity) => draw(entity, combat, boss))}
        </div>

        <div className={styles.hud}>
          <div className={styles.lives}>
            {Array.from({ length: lives }, (_unused, index) => <LifeIcon key={index} />)}
          </div>

          <p className={styles.round}>{`ROUND ${round}`}</p>

          {boss && (
            <HealthBar
              hp={boss.hp}
              maxHp={boss.maxHp}
              aiming={boss.stance === 'winding' && boss.attack === 'beam'}
              shielded={boss.stance === 'entering'}
            />
          )}

          <FrameMeter fps={rate.fps} worst={rate.worst} />

          {phase !== 'gameover' && (
            <button
              type="button"
              className={styles.pause}
              aria-label={phase === 'paused' ? 'Resume' : 'Pause'}
              onClick={onPause}
            >
              {phase === 'paused' ? '▶' : '❚❚'}
            </button>
          )}
        </div>

        {/* Above the field, so it catches touches in the margins as well. */}
        <div ref={surface} className={styles.surface} />

        <TouchStick ref={stick} />

        {phase === 'paused' && (
          <div className={styles.overlay}>
            <p className={styles.overlayTitle}>PAUSED</p>
            <div className={styles.overlayActions}>
              <button type="button" className={styles.action} onClick={onPause}>
                RESUME
              </button>
              <button type="button" className={styles.action} onClick={onQuit}>
                QUIT
              </button>
            </div>
          </div>
        )}

        {phase === 'gameover' && (
          <div className={styles.overlay}>
            <p className={styles.overlayTitle}>GAME OVER</p>
            <p className={styles.reached}>{`REACHED ROUND ${round}`}</p>
            <div className={styles.overlayActions}>
              <button type="button" className={styles.action} onClick={onQuit}>
                TITLE
              </button>
            </div>
          </div>
        )}
      </div>
    </GameProvider>
  );
}
